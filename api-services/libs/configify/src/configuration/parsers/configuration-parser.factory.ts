import type { ConfigurationParser } from '../configuration-parser.interface';
...
export class ConfigurationParserFactory {
  ...
  static getParser(file: string): ConfigurationParser {
    const ext = ConfigurationParserFactory.getFileExt(file);
    return ConfigurationParserFactory.parsers[ext];
  }
  ...
  static supports(file: string): boolean {
    ...
    return ConfigurationParserFactory.parsers.hasOwnProperty(ext);
  }
