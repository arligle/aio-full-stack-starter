...
export class ConfigifyModule {
  ...
  static async forRootAsync(
    ...
    const files = ConfigifyModule.resolveConfigurationFiles(settings.configFilePath);

    const envVars = settings.ignoreEnvVars ? {} : process.env;
    const fromFile = settings.ignoreConfigFile
      ? {}
      : ConfigifyModule.parseConfigurationFiles(files);

    const container = { ...envVars, ...fromFile };
    const secrets = await ConfigifyModule.runSecretsResolverPipeline(container, settings);
    ...
    const { exports, providers } = ConfigifyModule.buildConfigurationProviders();
  ...
  private static async runSecretsResolverPipeline(
    ...
    const secrets = {};
    for (const buildResolver of ConfigifyModule.SECRETS_RESOLVER_PIPELINE) {
  ...
  private static flattenObjectKeys(
    ...
    if (typeof source === 'object') {
      for (const key in source) {
        ConfigifyModule.flattenObjectKeys(source[key], [...path, key], target);
      }
    } else {
  ...
  private static parseConfigurationFiles(files: string[]): Record<string, any> {
    ...
    ConfigifyModule.flattenObjectKeys(config, [], kv);
  ...
  private static resolveConfigurationFiles(path?: string | string[]): string[] {
    return []
      .concat(path, ConfigifyModule.DEFAULT_CONFIG_FILES)
      .filter(
        (file) =>
