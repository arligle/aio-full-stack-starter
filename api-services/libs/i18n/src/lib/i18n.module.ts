...
import { APP_INTERCEPTOR, type HttpAdapterHost } from '@nestjs/core';
...
export class I18nModule implements OnModuleInit, NestModule {
  ...
  static forRoot(options: I18nOptions): DynamicModule {
    ...
    options = I18nModule.sanitizeI18nOptions(options);
    ...
    return {
      ...
      providers: [
        ...
        resolversProvider,
        i18nLoaderProvider,
        ...I18nModule.createResolverProviders(options.resolvers),
      ],
  ...
  static forRootAsync(options: I18nAsyncOptions): DynamicModule {
    ...
    const asyncOptionsProvider = I18nModule.createAsyncOptionsProvider(options);
    const asyncTranslationProvider = I18nModule.createAsyncTranslationProvider();
    const asyncLanguagesProvider = I18nModule.createAsyncLanguagesProvider();
    const asyncLoadersProvider = I18nModule.createAsyncLoadersProvider();
    ...
    if (options.useFactory) {
      ...
      return {
        ...
        useFactory: async (...args) => {
          ...
          return I18nModule.sanitizeI18nOptions(resolvers);
        },
    ...
    if (existingOrClass) {
      return {
        ...
        useFactory: async (optionsFactory: I18nOptionsFactory) =>
          I18nModule.sanitizeI18nOptions(
  ...
  private static createResolverProviders(resolvers?: I18nOptionResolver[]) {
    if (!resolvers || resolvers.length === 0) {
      logger.error(
        `No resolvers provided! @ifckit/i18n won't work properly, please follow the quick-start guide: https://docs.softkit.dev/libraries/i18n/quick-start`,
      );
    }
    return (resolvers || [])
