...
import type { I18nTranslator, I18nValidationError } from './interfaces';
...
export class I18nContext<K = Record<string, unknown>>
  ...
  static create(ctx: I18nContext, next: (...args: any[]) => void): void {
    I18nContext.storage.run(ctx, next);
  }
  ...
  static async createAsync<T>(
    ...
    return I18nContext.storage.run(ctx, next);
  }

  static current<K = Record<string, unknown>>(
    context?: ArgumentsHost,
  ): I18nContext<K> | undefined {
    const i18n = I18nContext.storage.getStore() as I18nContext<K> | undefined;
