...
<h3 align="center">再也不用写字符串来读取配置了</h3>
...
- 提供类型安全且直观的方式来配置您的 [Nest](https://github.com/nestjs/nest) 项目，就像要求DTO一样顺利。
- 加载您的配置,通过环境变量、以及json/yaml/toml文件或者远程端点.
...
`Nest-typed-config` 将默认安装所有加载器的依赖项。如果您关心依赖项大小和引导时间，请查看 [the guide to skip optional dependencies](./OPTIONAL-DEP.md).

## Inspiration

【Nest框架】有各种流行的配置模块(https://github.com/nestjs/nest), such as the [official configuration module](https://github.com/nestjs/config), [nestjs-config](https://github.com/nestjsx/nestjs-config) and [nestjs-easyconfig](https://github.com/rubiin/nestjs-easyconfig). 这些模块可以帮助管理配置、验证配置并通过“ConfigService”加载它们。但那时类型安全就消失了。For example:
...
编写类型转换是一件痛苦且难以维护的事情，并且在现实项目中使用非字符串配置是很常见的。该模块旨在提供一种直观且类型安全的方式来加载、验证和使用配置。只需导入任何配置模型，并通过完整的 TypeScript 支持注入它。 简而言之:
...
创建配置文件后，导入`TypedConfigModule`和`fileLoader`以从文件系统加载配置。
